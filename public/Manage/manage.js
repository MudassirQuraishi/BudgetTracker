...
async function getPagination(page) {
	try {
		...
		const response = await axios.get(
			`http://localhost:4000/expense/paginatedExpense?count=${count}&page=${currentPage}`,
			{ headers: { Authorization: token } },
		);
...
async function showPagination(data) {
	console.log(data);
	const { currentpage, nextPage, previousPage, hasNextPage, hasPreviousPage, lastPage } = data;
...
async function getPreviousDownloads(e) {
	const token = localStorage.getItem("token");
	try {
		const response = await axios.get("http://localhost:4000/premium/get-previous-reports", {
			headers: { Authorization: token },
		});
...
async function updateDashboard() {
	const token = localStorage.getItem("token");
	try {
		const response = await axios.get("http://localhost:4000/premium/dashboard", {
			headers: { Authorization: token },
		});
		const expenseData = {
