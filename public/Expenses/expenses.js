...
async function addIncome(e) {
	...
	if (income.value === "") {
		...
		const response = await axios.post("http://localhost:4000/expense/add-income", incomeDetails, {
...
async function addExpense(e) {
	...
	if ((expenseAmount.value === "") | (category.value === "")) {
		...
		try {
			...
			const response = await axios.post("http://localhost:4000/expense/add-expense", expenseDetails, {
				headers: { Authorization: token },
			});

			if (userList.childElementCount < 5) {
				createLiElement(response.data.data[0]);
			} else {
				description.value = "";
				expenseAmount.value = "";
...
async function createLiElement(userData) {
	...
	li.appendChild(document.createTextNode(`${userData.price} spent on ${userData.description} `));
...
const openRazorpay = async (data) => {
	...
	try {
		const options = {
			...
			handler: async function (response) {
				const update = await axios.post(
					"http://localhost:4000/purchase/updateMembership",
					{
						order_id: data.order.id,
		...
		rzp.on("payment.failed", async (failedData) => {
			const data = await axios.post("http://localhost:4000/purchase/failed", failedData, {
				headers: { Authorization: token },
			});
		});
	} catch (err) {
...
async function downloadReport(e) {
	...
	if (premiumStatus === "true") {
		try {
			const response = await axios.get("http://localhost:4000/premium/downloadReport", {
				headers: {
					Authorization: token,
				},
			});
	...
	try {
		...
		if (premiumStatus === "true") {
			...
			userData.forEach((user) => {
				const li = document.createElement("li");
				li.textContent = `Name:${user.userName}--Expense :${user.totalExpense}`;
